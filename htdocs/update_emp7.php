...
$tb       = "employees";  
$create_table_sql = "create table employees (employeeid char(4),lastname char(40),firstname char(40),title char(40),birthdate char(40))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=a size=40 value='". $o->firstname . "' $chg'></td>
      <td><input name=b size=20 value='". $o->lastname . "' $chg;text-align:right'></td>
    <td><input name=c size=20 value='". $o->title . "' $chg;text-align:right'></td>
    <td><input name=d size=20 value='". $o->birthdate . "' $chg;text-align:right'></td>
      <td>";
  ...
  echo "<tr><form action='' method=post><td><input name=id size=5></td><td><input name=a size=40></td><td><input name=b size=20></td><td><input name=c size=20></td></td><td><input name=d size=20></td><td><input type=submit name=action value='add' style='height:26'></td><td><button type='submit' formaction='employees.php'>Click Back</button></td></tr>
  </form></table>";
...
if ($act == "del : confirm") {
  $q  = "delete from $tb where employeeid ='". $_REQUEST{'id'} . "'";
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
...
if ($act == "edit : confirm") {
  $q  = "update $tb set  firstname ='". $_REQUEST{'a'} . "', lastname ='". $_REQUEST{'b'} . "', title ='". $_REQUEST{'c'} . "', birthdate ='". $_REQUEST{'d'} . "' where employeeid =" . $_REQUEST{'id'};
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
