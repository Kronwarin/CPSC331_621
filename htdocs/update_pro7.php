...
$tb       = "products";  
$create_table_sql = "create table products(productid int(4),productname varchar(40),supplierid char(40),categoryid char(40),quantityperunit char(40))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=a size=40 value='". $o->productname . "' $chg'></td>
      <td><input name=b size=20 value='". $o->supplierid . "' $chg;text-align:right'></td>
    <td><input name=c size=20 value='". $o->categoryid . "' $chg;text-align:right'></td>
    <td><input name=d size=20 value='". $o->quantityperunit . "' $chg;text-align:right'></td>
      <td>";
  ...
  echo "<tr><form action='' method=post><td><input name=id size=5></td><td><input name=a size=40></td><td><input name=b size=20></td><td><input name=c size=20></td></td><td><input name=d size=20></td><td><input type=submit name=action value='add' style='height:26'></td><td><button type='submit' formaction='product.php'>Click Back</button></td></tr>
  </form></table>";
...
if ($act == "del : confirm") {
  $q  = "delete from $tb where productid ='". $_REQUEST{'id'} . "'";
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
...
if ($act == "edit : confirm") {
  $q  = "update $tb set productname ='". $_REQUEST{'a'} . "', supplierid ='". $_REQUEST{'b'} . "', categoryid ='". $_REQUEST{'c'} . "', quantityperunit ='". $_REQUEST{'d'} . "' where productid =" . $_REQUEST{'id'};
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
