...
$tb       = "suppliers";  
$create_table_sql = "create table suppliers (supplierid int(4),companyname char(40),contactname char(40),contacttitle char(40),address char(40))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    if (isset($_REQUEST{'id'}) && $_REQUEST{'id'}  == $o->supplierid) $chg = " style='background-color:#f9f9f9"; else $chg = " readonly style='background-color:#ffffdd";
    ...
    <td><input name=d size=20 value='". $o->address . "' $chg;text-align:right'></td>
      <td>";
    if (isset($_REQUEST{'id'}) && $_REQUEST{'id'} == $o->supplierid) {
  ...
  echo "<tr><form action='' method=post><td><input name=id size=5></td><td><input name=a size=40></td><td><input name=b size=20></td><td><input name=c size=20></td></td><td><input name=d size=20></td><td><input type=submit name=action value='add' style='height:26'></td><td><button type='submit' formaction='suppliers.php'>Click Back</button></td></tr>
  </form></table>";
...
if ($act == "del : confirm") {
  $q  = "delete from $tb where supplierid ='". $_REQUEST{'id'} . "'";
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
...
if ($act == "edit : confirm") {
  $q  = "update $tb set companyname ='". $_REQUEST{'a'} . "', contactname ='". $_REQUEST{'b'} . "', contacttitle ='". $_REQUEST{'c'} . "', address ='". $_REQUEST{'d'} . "' where supplierid =" . $_REQUEST{'id'};
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
