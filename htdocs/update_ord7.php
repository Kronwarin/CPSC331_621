...
$tb       = "orders";  
$create_table_sql = "create table orders (orderid char(4),customerid char(40),employeeid char(40),orderdate char(40),requireddate char(40))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=a size=40 value='". $o->customerid . "' $chg'></td>
      <td><input name=b size=20 value='". $o->employeeid . "' $chg;text-align:right'></td>
    <td><input name=c size=20 value='". $o->orderdate . "' $chg;text-align:right'></td>
    <td><input name=d size=20 value='". $o->requireddate . "' $chg;text-align:right'></td>
      <td>";
  ...
  echo "<tr><form action='' method=post><td><input name=id size=5></td><td><input name=a size=40></td><td><input name=b size=20></td><td><input name=c size=20></td></td><td><input name=d size=20></td><td><input type=submit name=action value='add' style='height:26'></td><td><button type='submit' formaction='orders.php'>Click Back</button></td></tr>
  </form></table>";
...
if ($act == "edit : confirm") {
  $q  = "update $tb set customerid ='". $_REQUEST{'a'} . "', employeeid ='". $_REQUEST{'b'} . "', orderdate ='". $_REQUEST{'c'} . "', requireddate ='". $_REQUEST{'d'} . "' where orderid =" . $_REQUEST{'id'};
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
