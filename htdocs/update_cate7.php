...
Connc();
function Connc() {
  global $host,$db,$tb,$user,$password,$conn,$create_table_sql;
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    ...
    echo "<tr><form action='' method=post>
      ...
      <td><input name=a size=40 value='". $o->categoryname . "' $chg'></td>
  ...
  echo "<tr><form action='' method=post><td><input name=id size=5></td><td><input name=a size=40></td><td><input name=b size=20></td><td><input type=submit name=action value='add' style='height:26'></td><td><button type='submit' formaction='categories.php'>Click Back</button></td></tr>
  </form></table>";
